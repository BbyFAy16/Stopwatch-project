document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStats() {
    ...
    lapStats.textContent = `Avg: ${formatTime(avg)} | Best: ${formatTime(best)}`;
  }

  // Lap highlight
  function highlightLaps() {
    if (laps.length < 3) return;
    laps.forEach((lap) => lap.row.classList.remove("lap-fastest","lap-slowest","lap-normal"));
    laps[0].row.classList.remove("lap-fastest","lap-slowest","lap-normal");
  ...
  function addLap() {
    ...
    tr.innerHTML = `<td>${lapNumber}</td><td>${formatTime(split)}</td><td>${formatTime(elapsed)}</td>`;
